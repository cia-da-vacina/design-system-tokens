...
/**
 * Semantic color keys resolved by styled-system (bg="bg.brand.solid").
 * Pipeline stages are first-class for the CRM inbox.
 */
const colors = {
  // backgrounds
  ...
  // legacy aliases used by raw palette access
  ...flattenRaw(raw),
} as const;
...
function flattenRaw(palette: typeof raw): Record<string, string> {
  ...
  for (const [group, scale] of Object.entries(palette)) {
    ...
    for (const [step, value] of Object.entries(scale)) {
      out[`${group}.${step}`] = value as string;
    }
  }
  return out;
}
